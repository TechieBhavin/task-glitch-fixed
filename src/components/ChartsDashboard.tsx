...
const roiBuckets = [
  { label: '<200', count: tasks.filter(t => Number.isFinite(t.roi) && (t.roi as number) < 200).length },
  { label: '200-500', count: tasks.filter(t => Number.isFinite(t.roi) && (t.roi as number) >= 200 && (t.roi as number) <= 500).length },
  { label: '>500', count: tasks.filter(t => Number.isFinite(t.roi) && (t.roi as number) > 500).length },
  { label: 'N/A', count: tasks.filter(t => !Number.isFinite(t.roi)).length },
];
